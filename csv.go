...
// thusly:
//
//   import "github.com/goex-top/csv-rotate"
//
...
func (c *Csv) openNew() error {
	...
	if len(c.CsvHeader) > 0 { // write csv header
		err = c.csv.Write(c.CsvHeader)
		if err != nil {
			return fmt.Errorf("can't write header: %s", err)
		}
	}
...
func (c *Csv) millRunOnce() error {
	if c.MaxBackups == 0 && c.MaxAge == 0 && !c.Compress {
		return nil
	}

	files, err := c.oldLogFiles()
	if err != nil {
		return err
	}
	...
	for _, f := range remove {
		errRemove := os.Remove(filepath.Join(c.dir(), f.Name()))
		if err == nil && errRemove != nil {
			err = errRemove
		}
	}
	for _, f := range compress {
		fn := filepath.Join(c.dir(), f.Name())
...
func (c *Csv) timeFromName(filename, prefix, ext string) (time.Time, error) {
...
func (c *Csv) dir() string {
	return filepath.Dir(c.filename())
}

// prefixAndExt returns the filename part and extension part from the Logger's
// filename.
func (c *Csv) prefixAndExt() (prefix, ext string) {
